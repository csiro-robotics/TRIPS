...
def compute_ewc_importance(model, eval_meta, test_envs):
    precision_matrix_dict = {}
    final_precision_matrix = {}
    num_of_used_dataset = 0
    
    print('--- compute_ewc_importance ---')
    for name, loader_kwargs, weights in eval_meta:
        ...
        precision_matrix = ewc_importance_matrix(model, loader)
...
def ewc_importance_matrix(model, data_loader):
    # max_importance = 0
    # min_importance = 100
    precision_matrices = {}
    ...
    """
    for name, value in precision_matrices.items():
        if torch.max(value) > max_importance:
            max_importance = torch.max(value)
            max_importance_name = name
        if 0 < torch.min(value) < min_importance:
            min_importance = torch.min(torch.min(value))
            min_importance_name = name
    print('!!!max importance!!! | name: {0}, value: {1}'.format(max_importance_name, max_importance))
    print('!!!min importance!!! | name: {0}, value: {1}'.format(min_importance_name, min_importance))
    """
...
class EWC(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, current_session, old_parameters, precision_matrix, hparam_lambda=10000):
        super(EWC, self).__init__(input_shape, num_classes, num_domains, hparams, current_session)
    ...
    def ewc_penalty(self, model, old_parameters, precision_matrix):
        ewc_loss = 0
    ...
    def update_incremental(self, x, y, **kwargs):
        ...
        ewc_loss = self.ewc_penalty(self.network, self.old_parameters, self.precision_matrix)
        total_loss = cross_entropy_loss + self.hparam_lambda * ewc_loss
        # print('ewc_loss: {0} * {1}, cross_entropy_loss: {2}, total_loss: {3}'.format(self.hparam_lambda, ewc_loss, cross_entropy_loss, total_loss))
